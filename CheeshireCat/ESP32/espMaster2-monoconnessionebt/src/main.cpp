...
// INIZIO GANGSTER CODE - PULITO E PRONTO A GIRARE
...
String ricezioneEV3(String& default_istruzione, bool& default_checkInvio) {
  ...
  while (gpsSerial.available()) {
    char gpsData = gpsSerial.read();
    if (index < sizeof(Json) - 1) Json[index++] = gpsData;
  }

  if (index > 0 && Json[index - 1] == '!') index--;
  Json[index] = '\0';
  ...
  if (index > 0) {
    ...
    default_istruzione = msg_json["istruzione"].as<String>();
    default_checkInvio = msg_json["checkInvio"];
  }
  return "";
}
...
String ricezioneRaspi(bool& checkInvio) {
  ...
  while (Serial.available()) {
    char data = Serial.read();
    if (index < sizeof(Json) - 1) Json[index++] = data;
  }
  ...
  if (index > 0) {
    ...
    checkInvio = msg_json["checkInvio"];
    return msg_json["istruzione"].as<String>();
  }
  return "";
}

void serializzaJson(int tipo, int ID, String istruzione, char* buffer, size_t bufSize) {
  StaticJsonDocument<255> msg_json;
  msg_json["tipo"] = tipo;
  msg_json["ID"] = ID;
  msg_json["istruzione"] = istruzione;
  ...
  size_t len = serializeJson(msg_json, buffer, bufSize);
  if (len < bufSize - 1) buffer[len++] = '!';
  buffer[len] = '\0';
}

void invioEV3(int tipo, int ID, String istruzione) {
  serializzaJson(tipo, ID, istruzione, Json, sizeof(Json));
  gpsSerial.write((const uint8_t*)Json, strlen(Json));
}

void invioRaspi(int tipo, int ID, String istruzione) {
  serializzaJson(tipo, ID, istruzione, Json, sizeof(Json));
  Serial.write((const uint8_t*)Json, strlen(Json));
}

void invioUARTesp(int tipo, int ID, String istruzione) {
  serializzaJson(tipo, ID, istruzione, Json, sizeof(Json));
  ESPSerial.write((const uint8_t*)Json, strlen(Json));
}
...
void setup() {
  Serial.begin(115200);
  gpsSerial.begin(GPS_BAUD, SERIAL_8N1, RXD2, TXD2);
  ESPSerial.begin(ESP_BAUD, SERIAL_8N1, RXD1, TXD1);
...
void sano(bool sens) {
  digitalWrite(M1orario, sens);
  digitalWrite(M2orario, sens);
  digitalWrite(M1antiorario, !sens);
  digitalWrite(M2antiorario, !sens);
}
...
String assegnaNome(int id) {
  if (id == 1) return slave1;
  if (id == 2) return slave2;
  if (id == 3) return slave3;
  return "";
}

void invioSicuroEsp(int id, String istruzione) {
  String slaveName = assegnaNome(id);
  if (SerialBT.connect(slaveName)) {
    serializzaJson(2, id, istruzione, Json, sizeof(Json));
    digitalWrite(BUILTIN_LED, HIGH);
    SerialBT.write((const uint8_t*)Json, strlen(Json));

    unsigned long start = millis();
    while (millis() - start < 15000) {
      if (SerialBT.available()) {
        String messaggio = SerialBT.readStringUntil('\n');
        StaticJsonDocument<255> msg_json;
        if (!deserializeJson(msg_json, messaggio)) {
          String istr = msg_json["istruzione"];
          bool check = msg_json["checkInvio"];
          if (istr == istruzione && check) break;
        }
      }
    }
    digitalWrite(BUILTIN_LED, LOW);
...
void invioSicuro(int tipo, int ID, String istruzione) {
  if (tipo == 2) {
    if (ID == 1)
      ...
      bool check = false;
      invioRaspi(tipo, ID, istruzione);
      if (ricezioneRaspi(check) == istruzione && check) {
        digitalWrite(BUILTIN_LED, HIGH);
        delay(100);
        digitalWrite(BUILTIN_LED, LOW);
      }
    ...
    bool checkInvio = false;
    unsigned long start = millis();
    while ((millis() - start < 15000) && (!checkInvio || ist != istruzione)) {
      ricezioneEV3(ist, checkInvio);
    }
    if (checkInvio && ist == istruzione) {
      digitalWrite(BUILTIN_LED, HIGH);
      delay(100);
      digitalWrite(BUILTIN_LED, LOW);
    }
