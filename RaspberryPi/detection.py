...
from Comm.CommDefault import *
import Comm.CommDefault
...
def main():
    ...
    sequenza = ["-cover-", "-rabbit-", "-cat-", "-caterpillar-", "-hatman-", "-queen-", "-alice-"]
    index = 0
    audioPath = "audio/"
    qrAudio = "qrcode.mp3"
    audios = ["cover.mp3", "rabbit.mp3", "cat.mp3", "caterpillar.mp3", "hatman.mp3", "queen.mp3", "alice.mp3"]
    ...
    while True:
        ...
        if qrDetected:
            ...
            if sequenza[index] == current_prediction:
                playsound(audioPath + audios[index])
                index += 1
                #send_until_success(esp, 0, 0, current_prediction)
            ...
            if vertices_array is not None:
                ...
                if len(vertices_array) > 0:
                    ...
                    vertices = np.array(vertices_array[0], dtype=np.int32)
            ...
            if data == "START":
                qrDetected = True
                print("QR detected, time to start the model")
                playsound(audioPath + qrAudio)
